defmodule PlausibleWeb.GoogleAnalyticsController do
  ...
  def property_form(
    ...
    redirect_route = ~p"/#{PlausibleWeb.SitePath.encode(site.domain)}/settings/imports-exports"
  ...
  def property(
    ...
    with {:ok, api_start_date} <- Google.API.get_analytics_start_date(access_token, property),
      ...
      redirect(conn,
        to:
          ~p"/#{PlausibleWeb.SitePath.encode(site.domain)}/import/google-analytics/confirm?#{[property: property, access_token: access_token, refresh_token: refresh_token, expires_at: expires_at, start_date: Date.to_iso8601(start_date), end_date: Date.to_iso8601(end_date)]}"
      )
    else
