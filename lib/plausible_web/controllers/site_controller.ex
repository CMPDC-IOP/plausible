defmodule PlausibleWeb.SiteController do
  ...
  def create_site(conn, %{"site" => site_params}) do
    ...
    case Sites.create(user, site_params, team) do
      {:ok, %{site: site}} ->
        ...
        redirect(conn,
          to:
            Routes.site_path(conn, :installation, PlausibleWeb.SitePath.encode(site.domain),
              flow: flow
            )
        )
  ...
  def settings(conn, %{"domain" => domain}) do
    redirect(conn,
      to: Routes.site_path(conn, :settings_general, PlausibleWeb.SitePath.encode(domain))
    )
  end
  ...
  def update_settings(conn, %{"site" => site_params}) do
    ...
    case Repo.update(changeset) do
      {:ok, site} ->
        ...
        |> redirect(
          to: Routes.site_path(conn, :settings_general, PlausibleWeb.SitePath.encode(site.domain))
        )
  ...
  def reset_stats(conn, _params) do
    ...
    |> redirect(
      to: Routes.site_path(conn, :settings_danger_zone, PlausibleWeb.SitePath.encode(site.domain))
    )
  end
  ...
  def make_public(conn, _params) do
    ...
    |> redirect(
      to: Routes.site_path(conn, :settings_visibility, PlausibleWeb.SitePath.encode(site.domain))
    )
  end
  ...
  def enable_weekly_report(conn, _params) do
    ...
    |> redirect(
      to:
        Routes.site_path(conn, :settings_email_reports, PlausibleWeb.SitePath.encode(site.domain))
    )
  end
